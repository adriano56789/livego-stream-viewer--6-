...
const AppContent: React.FC = () => {
  ...
  // Initialize Verification Agent on App Mount
  useEffect(() => {
    ...
    if (isAuthenticated && !currentUser) { // Only fetch if authenticated but user is not loaded
      api.getCurrentUser().then(async user => {
        if (user) {
            ...
            // Initial fetch of conversations to get unread counts immediately
            api.getConversations(user.id).then(data => setConversations(data || [])).catch(console.error);
  ...
  // ... (keeping existing useEffects for websocket events) ...
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // DIRECT INVITE HANDLING: Now directly opens GoLiveScreen
    const handlePrivateInvite = (payload: { streamId: string, hostId: string, streamName: string, inviterName: string, inviterAvatar: string }) => {
         // Open GoLiveScreen directly instead of showing a notification
  ...
  // ... (keeping existing useEffects for loading messages, visitors, stream room data, stream updates, live session updates, user updates, transaction updates) ...
  useEffect(() => {
    // Only refresh conversations if active screen is messages, OR we want to update badges
    if (currentUser) {
        api.getConversations(currentUser.id).then(data => setConversations(data || [])).catch(err => {
            console.error("Failed to load conversations:", err);
            // Don't toast error on background poll
        });
  ...
  }, [activeScreen, currentUser, addToast, chattingWith]); // Trigger refresh when closing a chat to update read status
  ...
  // ... (WebSocket Handlers for follow, mic, sound, user, transaction updates) ...
  useEffect(() => {
    ...
    const handleNewMessage = (message: Message) => {
        ...
        setConversations(prevConversations => {
            ...
            if (index > -1) {
                ...
                // If it's a new conversation, we might need to fetch it or construct it.
                // For simplicity in this mock, we trigger a fetch
                api.getConversations(currentUser.id).then(data => setConversations(data || []));
    ...
    webSocketManager.on('newMessage', handleNewMessage); // Listen for private messages globally
...
const handleDenyLocation = async () => {
  ...
  const checkMicrophonePermission = async () => {
    ...
    try {
        ...
        if (status === 'denied') {
             // Fix: Reset to prompt for demo purposes to allow retry
             await api.updateMicrophonePermission(currentUser.id, 'prompt');
  ...
  const checkCameraPermission = async () => {
      ...
      try {
          ...
          if (status === 'denied') {
              // Fix: Reset to prompt for demo purposes to allow retry
              await api.updateCameraPermission(currentUser.id, 'prompt');
  ...
  // ... (handleSelectStream, handleStartStream, handleRequestEndStream, handleConfirmEndStream, etc.) ...
  ...
  const handleStartStream = async (streamer: Streamer) => {
    ...
    setPrivateInviteData(null); // Clear invite data after using it
    ...
    if (currentUser && streamer.hostId === currentUser.id) {
        // Use new endpoint to trigger notifications
        const { success } = await api.startLiveStream(streamer.id);
        ...
        if (!success) {
            addToast(ToastType.Error, "Aviso: Notificações de início de live podem ter falhado.");
        } else {
             // Show notification modal to the streamer themselves (preview)
             setLiveNotification({
        ...
        // Refetch lists to ensure global consistency
        const countryCode = selectedCountry === 'ICON_GLOBE' ? undefined : selectedCountry;
        api.getLiveStreamers(activeCategory, countryCode).then(data => setStreamers(data || []));
        api.getReminders().then(data => setReminderStreamers(data || []));
    }
  };
  ...
  // Updated handleConfirmPurchase to show PaymentSuccessScreen
  const handleConfirmPurchase = async (pkg: { diamonds: number; price: number }) => {
      ...
      try {
          ...
          if (success && user) {
              updateUserEverywhere(user);
              // Instead of just toast, setup success data and open the screen
              setPaymentSuccessData({
                  price: pkg.price,
                  diamonds: pkg.diamonds,
                  method: 'pix', // Assuming Pix for the demo based on the visual
                  timestamp: new Date()
              });
  ...
  // ... (Other handlers like handlePurchaseFrame, handleOpenListScreen, etc.) ...
  ...
  const handleWatchLiveNotification = async () => {
    ...
    // Standard public notification flow
    // Find the streamer in the current list, or create a temporary object
    ...
    if (!targetStream) {
        // Fallback: try to construct a stream object if not in the visible list (e.g. filtered out)
        targetStream = {
            id: targetId, // Use specific ID if available
            ...
            tags: [],
            isPrivate: !!liveNotification.isPrivate, // Respect private flag if passed
            country: 'br', // Default
            viewers: 0
        };
        
        // Try to fetch real stream data if possible (optional optimization)
        try {
           // We might need to fetch from private endpoint if it's a private room
           const liveStreamers = await api.getLiveStreamers(liveNotification.isPrivate ? 'private' : 'popular'); 
           const found = liveStreamers.find(s => s.id === targetId);
           if (found) targetStream = found;
        } catch (e) {
           console.warn("Could not fetch latest stream list for notification", e);
        }
    }
    ...
    <div className="h-full w-full bg-black text-white overflow-hidden relative font-sans">
      ...
      {activeStream && streamRoomData && currentUser ? (
        ...
        <>
          ...
          <FooterNav currentUser={currentUser} onOpenGoLive={handleOpenGoLive} activeTab={activeScreen} onNavigate={handleNavigation} unreadCount={totalUnreadMessages} />
        </>
      )}
