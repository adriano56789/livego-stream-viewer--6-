...
import { BackIcon, CreditCardIcon, GoldCoinWithGIcon, PixIcon, LockIcon, QuestionMarkIcon, CopyIcon, CheckCircleIcon } from './icons';
  ...
  useEffect(() => {
      const initOrder = async () => {
          try {
              ...
              const order = await api.createOrder(CURRENT_USER_ID, 'pkg_custom', packageDetails.price, packageDetails.diamonds);
              setOrderId(order.id);
    ...
    if (paymentMethod === 'credit_card') {
        ...
        try {
            ...
            if (paymentResult.success) {
                // Confirm purchase to grant diamonds
                await api.confirmPurchase(orderId);
