...
const MainSettingsPage: React.FC<{ navigateTo: (page: string) => void; onLogout: () => void; onOpenLanguageModal: () => void; }> = ({ navigateTo, onLogout, onOpenLanguageModal }) => {
    ...
    // Developer Tool: Simulate Live Notification
    const simulateLiveNotification = () => {
        // Pick a random streamer from DB
        const randomStreamer = db.streamers[Math.floor(Math.random() * db.streamers.length)];
        if (randomStreamer) {
            webSocketServerInstance.notifyStreamerGoesLive(randomStreamer, false);
        }
    };
